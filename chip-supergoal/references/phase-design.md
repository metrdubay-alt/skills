...
**Pre-flight pitfall:** do not put commands in a phase spec that necessarily fail because the phase has not created the referenced file yet, then treat that as a baseline red. If a future file is the point of phase 1, make its mandatory command a post-phase command inside that phase, and keep pre-flight to existing repo-level gates. When repo-level raw typecheck is already red from unrelated test debt but the framework build/typecheck is green, use the green framework build plus focused tests/lint for changed files as the acceptance gate, and explicitly record the unrelated baseline debt instead of widening the phase into a repo-wide cleanup.
